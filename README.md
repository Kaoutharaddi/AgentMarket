...
verify_and_pay()
  ...
  └── 5. CPI → RISC Zero VerifierRouter
            ├── image_id = AGENT_GUEST_ID         (hardcoded — caller cannot override)
...
- `image_id` is hardcoded as `AGENT_GUEST_ID` in the contract — callers cannot supply their own value, so only proofs generated by the authorized guest are accepted
- `cancel_job` is only callable by the original client (enforced by `job.client == client.key()`) — after 72h it can also cancel jobs in `InProgress` state, protecting clients from unresponsive agents
